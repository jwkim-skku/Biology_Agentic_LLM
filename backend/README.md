...
- `GET /api/v1/optimizer/benchmark/export.zip`: export a manifested optimizer benchmark bundle with benchmark metrics, stable result hashes, case provenance fingerprints, recommendation summary trade-off/Pareto evidence, recommended-candidate folding evidence hashes, recommendation trade-off/regret columns, diagnostics, stress gate, case catalog, candidate diagnostics, config snapshots, structured manifest, archive summary, and deployment-audit evidence.
...
- `GET /api/v1/rag/regression/export.zip`: export a manifested RAG regression suite bundle with results, stable result hashes, cases, metrics CSV, weak cases, source provenance summary, diagnostics, ranking policy, RAG status, and structured manifest.
