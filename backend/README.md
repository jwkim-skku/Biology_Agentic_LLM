...
- `GET /api/v1/optimizer/benchmark/export.zip`: export a manifested optimizer benchmark bundle with benchmark metrics, stable result hashes, case provenance fingerprints, recommendation trade-off/regret columns, diagnostics, stress gate, case catalog, candidate diagnostics, config snapshots, structured manifest, archive summary, and deployment-audit evidence.
