...
`scripts/compose_preflight.py` uses a synthetic production env with Postgres, signing, rate-limit, OpenAI embedding, object-store mirror, and external timestamp-provider values, so `docker compose config` checks the same required variables that production startup demands. Static-only runs also emit `static_evidence_hash`, compose-file SHA-256 values, required token hashes, and `synthetic_env_keys_hash` so CI/preflight artifacts preserve the exact deployment-shape checks even when Docker execution is unavailable.
...
python scripts/production_promotion_runbook.py --write-result backend/app/data/runtime/production_audit_write_result.json --format json --output backend/app/data/runtime/production_promotion_runbook.json
python scripts/verify_production_promotion_runbook.py --path backend/app/data/runtime/production_promotion_runbook.json
```
...
The audit writes paired JSON and Markdown reports to `backend/app/data/runtime/production_audits`. The full profile combines production env validation, Docker Compose deployment-shape checks, optional recent preflight evidence validation, and live API checks for health readiness, deployment readiness, production audit status payload integrity, production audit bundle verification, security, storage, data provenance, data-release bundle verification, external source coverage, RAG diagnostics, RAG embedding status, RAG evaluation/regression bundle verification, optimizer diagnostics, RNAfold backend status, optimizer benchmark bundle verification, QC report bundle request-provenance verification, governance attestation verification, artifact-ledger verification, object-store mirror backlog, and archived QC/structured-import/data-release/RAG/vector-index/optimizer/workflow-trace semantic summaries. The live bundle checks require API auth, RBAC bindings, at least one API key, positive rate limiting, artifact signing key ids, Postgres target/active storage with configured `DATABASE_URL`, Postgres schema hash, driver availability, vector extension readiness, deployment readiness gate/action hashes, production audit status `audit_hash`, gap-summary hash, resolution scope/mode counts, production audit verifier `semantic_summary` status/hash/count consistency, required action coverage/detail hashes, production RAG embeddings with `model_fingerprint_hash`, OpenAI price/budget guardrails when used, validated RNAfold execution backend, governance attestation hash plus verified signature and bundle file checks, artifact-ledger entries/latest hash with zero missing archived artifacts, data-release record hashes, tRNA caveat count/blocking-use evidence, RAG evaluation retrieval-trace/evidence-sufficiency/facet-gap/query-term/top-source coverage checks, the RAG regression result hash, archived RAG vector-index chunk/dimension/model/retrieval/manifest evidence, optimizer benchmark result hash plus seed-strategy/candidate-diagnostics/recommendation-audit checks, archived optimizer benchmark/stress/case/folding-candidate-match evidence, workflow trace step/hash/task evidence, object-store mirror plan readiness with zero pending candidates when enabled, and QC request/report/candidate-ranking hashes plus recommendation-readiness/recommended-folding candidate IDs, optimizer reproducibility, and candidate-ranking explainability checks before promotion. After live API checks finish, the CLI adds an `api_consistency` check that requires `/deployment/audit` and `/deployment/audit/verify` to report the same `audit_hash` and requires `/deployment/readiness` action hashes to match the readiness evidence embedded in the audit payload. The CLI JSON includes an `audit_hash`; the write-mode console output uses `agentic-rag-cli-production-audit-write-result-v1` and repeats the audit/preflight status, failure/warning counts, hashes, and JSON/Markdown SHA-256 values beside the artifact paths for CI log traceability. `verify_production_audit_write_result.py` reopens those JSON/Markdown artifacts, recomputes the audit hash, recomputes the summary from the audit checks, checks the write-result summary against the audit JSON, verifies the referenced preflight evidence hash/checks hash/status/counts against the original preflight JSON, and verifies the Markdown audit hash/status/count lines so CI artifacts cannot silently point to a different report. The promotion runbook renderer consumes either that write-result or an audit JSON, verifies every production gap hash, verifies the source proof-checklist count/hash/list against the gap list, emits a `proof_checklist_source_match` result, pins every proof row with `proof_item_hash`, and groups the remaining actions by resolution scope/mode for handoff. The runbook verifier recomputes the checklist from those grouped action rows, checks source timestamp/status metadata, checks scope/mode/blocking/promotion counts, verifies the optional Markdown runbook contains the same hashes/status/checklist rows, and fails if `production_ready=true` is reported while unresolved handoff gaps remain. The Markdown report repeats the hash, summarizes live API evidence status counts when API checks run, and includes the preflight evidence root, output JSON path, age, required-check count, missing-check count, declared/actual failed-check count, status-mismatch count, skipped-check count, and missing command/cwd/duration/details/output evidence counts for promotion review. When `--preflight-evidence` is supplied, stale, failing, or incomplete preflight JSON is treated as a production audit failure.
...
For a final portfolio handoff on a workstation, generate and verify the complete reviewer evidence set in one command:
...
This runs backend compile checks, production env template validation, Docker Compose deployment-shape checks, the static production audit profile, API contract checks, structured import CLI preview verification, GTEx/Allen data-refresh CLI plan verification, response/value golden tests, the final portfolio evidence check and its verifier, the manual backend regression suite, the frontend production build, the HTTP smoke test, a separate signed-artifact smoke profile with `ARTIFACT_SIGNING_KEY` enabled, and a browser UI smoke test that checks operational panels and metrics hydration. The API contract check pins every current FastAPI route and fails unexpected new `/api/v1` routes until they are added to `REQUIRED_PATHS`; it also requires JSON API operations to declare the shared `ApiResponse` envelope unless explicitly listed as raw export, download, text, or health responses. Use `--skip-frontend`, `--skip-smoke`, `--skip-signing-smoke`, or `--skip-ui-smoke` for narrower deployment diagnostics. CI generates `portfolio_readiness_matrix.json`, verifies its matrix hash, requirement evidence hashes, file SHA-256 values, and glob evidence counts with `verify_portfolio_readiness_matrix.py`, then uploads it as the `backend-portfolio-readiness-matrix` artifact. CI also writes `ci_production_evidence_chain.json`, verifies that the workflow still generates, verifies, and uploads every production evidence artifact, and uploads it as `backend-ci-production-evidence-chain`. The submission summary renderer combines the matrix and CI evidence chain into `portfolio_submission_summary.json` and `portfolio_submission_summary.md`; `verify_portfolio_submission_summary.py` recomputes the summary hash, validates matrix/CI evidence linkage, checks Markdown status/hash/coverage rows, and CI uploads both files as the `backend-portfolio-submission-summary` artifact for reviewer-friendly portfolio handoff. The final portfolio check regenerates the matrix, CI evidence chain, submission summary JSON/Markdown, validates each artifact, records SHA-256 values plus payload hashes, writes `final_portfolio_check.json`, verifies it with `verify_final_portfolio_check.py`, and CI uploads it as `backend-final-portfolio-check`. `--output-json` writes a timestamped `agentic-rag-production-preflight-evidence-v1` payload with the command, working directory, duration, required-check list/count, check/pass/fail/skip counts, pass/fail result, compact stdout/stderr, parsed JSON details for every check, `mode_hash`, `skipped_hash`, `checks_hash`, and `preflight_hash`; CI uploads this file as the `backend-preflight-evidence` artifact, then validates it through `production_audit.py --preflight-evidence`, verifies the audit write-result schema plus JSON/Markdown SHA-256 hashes with `verify_production_audit_write_result.py`, renders the promotion runbook Markdown/JSON from that write-result, verifies the runbook schema/hash/proof-checklist source match and Markdown/JSON consistency with `verify_production_promotion_runbook.py --markdown-path`, and uploads the resulting `backend-production-audit`, `backend-production-audit-write-result`, and `backend-production-promotion-runbook` artifacts. The production audit also checks that preflight structured-import, compose, and data-refresh details include source hashes, compose static evidence hashes, planned operations, dry-run status, validation schema, normalized gene/region requests, repository-root/output-path consistency, per-check command/cwd/duration/details/stdout/stderr evidence, matching preflight mode/skip/check/evidence hashes, skip-mode consistency, exact failed-list versus nonzero-returncode consistency, per-check status/returncode consistency, expected preflight schema/required-check coverage, and data-release bundle/archive evidence includes release-lock current and persisted hashes.
